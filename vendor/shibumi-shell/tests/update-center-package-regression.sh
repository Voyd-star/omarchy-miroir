...
status_script="$repo_root/hancore.shibumi.update-center/scripts/package-status"

fail() {
  printf 'update center package regression failed: %s\n' "$*" >&2
  exit 1
}
...
SH
chmod +x "$stub_bin/checkupdates" "$stub_bin/omarchy-cmd-missing"
...
mkdir -p "$minimal_bin"
for command in bash date dirname jq mktemp rm; do
  ln -s "$(command -v "$command")" "$minimal_bin/$command"
done
ln -s "$stub_bin/omarchy-cmd-missing" "$minimal_bin/omarchy-cmd-missing"

status=$(PATH="$minimal_bin" "$status_script")
